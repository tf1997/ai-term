...
- 命令始终在任务启动时的当前终端可见地执行(不开隐藏 shell),与产品"执行可审查、用户可控"原则一致。
- 停止按钮随时可中断:中断模型请求(复用取消令牌)且不再派发后续命令;已发出的命令不强行终止,任务收尾后释放终端接管。
- 普通对话模式行为与现状完全一致。
...
- 任务运行期间接管绑定终端:该终端拦截人工键盘、粘贴、快捷命令、脚本和普通派发,并禁止关闭标签；Agent 捕获器使用内部授权继续写入。用户仍可切换到其他终端执行命令，任务进入 done/stopped/error 后释放接管。
...
| 任务轮次(新增) | 80k 字符(前端 24k 软预算) | 新常量 `MAX_AGENT_TURN_CHARS`:超限时从最早轮次开始,把 `ToolResult.content` 压缩为 `{"exitCode":N,"note":"输出已省略"}`,`Assistant.text` 截断;最近 6 轮完整保留 |
...
| 2 | **上下文预算过小(最硬的伤)** | 旧轮次预算 24k 字符,单命令输出上限 4k → **6 条命令即撑满**,`compressAgentTurns` 开始把早期 toolResult 整体替换为「输出已省略」,只保护最近 3 轮;后端超限直接 bail | 后端 `MAX_AGENT_TURN_CHARS` 24k → 80k;前端 `DEFAULT_MAX_TURN_CHARS` 保持 24k 软预算(低于后端,保证前端先压缩而非后端报错);`PROTECTED_RECENT_TURNS` 3 → 6;压缩策略改为保留退出码 + 输出尾部片段,而非整体丢弃 |
...
- 后端硬上限提高到 80k,前端仍以 24k 软预算优先压缩旧证据;单次请求 token 仍可能上升,需在实测中确认成本与网关上限(部分网关对单请求有硬上限,超限报错文案已有 5.7 兜底)。
...
**关键性质:nonce 作为 `%s` 参数传入,而非写死在格式串里。** 组装后的标记 `__AI_TERM_<nonce>B__` 只可能出现在 `printf` 真正执行后的输出中;shell 对命令行的回显、`history`、`ps` 撞见我们自己这条进程,看到的都是含 `%s` 的字面文本,永远匹配不上。哨兵方案最主要的失败模式(回显串扰)由此结构性消失,不需要"跳过第一次出现"之类的脆弱启发式。结束标记额外要求冒号后是数字,字面的 `%d` 同样匹配不上,构成第二道保险。

前导 `\n` 保证标记独占一行(原命令输出不以换行结尾时仍成立)。`$?` 在参数展开时求值,此刻正是原命令的退出码。

- **能力探针**:首次需要哨兵时派发 `printf …; (exit 7); printf …`,8s 内收到结束标记且退出码恰为 7 才启用。这一条同时验证 `printf` 可用、`$?` 语义正确、标记能原样往返;fish / PowerShell / cmd 自然探针失败,**不需要猜 shell 方言**。结果按 sessionId 缓存,重连即失效。
- **启用方式**:探针通过即自动启用,无用户开关。有 OSC 133 标记时仍走原路径,哨兵不介入。
- **结构安全守卫**:`isSuffixSafeForSentinel` 复用 `agentAutoApprove.ts` 的引号感知扫描器,拒绝无法安全追加 `; printf` 的命令(未闭合引号、heredoc、结尾为 `&` / `|` / `&&` / 续行反斜杠),返回 `dispatch-failed` 并说明原因,模型会换写法重试。
- **无串扰检测**:结束标记只可能由我们派发的那一行产生,用户中途手敲的命令伪造不了,因此哨兵路径不设 `commandMismatch`。
- **命令历史记干净命令**:`executeCommand` 增加 `historyCommand` 替身参数,派发包装命令、历史记原命令。

已知限制(未在本期解决):

- 远端 `set -e` 时原命令失败会让 shell 先退出,收不到结束标记 → 走超时。交互式 shell 极少设。
