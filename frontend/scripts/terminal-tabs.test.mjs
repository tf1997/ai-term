...
import { normalizedTerminalTargetIds, terminalStatusClass } from '../src/lib/terminalTabs.ts'
import { useTerminalTabs } from '../src/composables/useTerminalTabs.ts'
...
test('连接代次只在进入远程或 SFTP 连接时递增', () => {
  const state = useTerminalTabs()
  for (const status of ['connecting', 'local', 'preview', 'error']) {
    state.updateTerminalStatus('local-1', status)
    assert.equal(state.activeTerminal.value.connectionGeneration, 0)
  }
  for (const status of ['remote', 'remote', 'sftp']) {
    state.updateTerminalStatus('local-1', status)
    assert.equal(state.activeTerminal.value.connectionGeneration, 1)
  }
  state.updateTerminalStatus('local-1', 'idle')
  state.updateTerminalStatus('local-1', 'sftp')
