...
import { useTerminalCapture } from '../application/useTerminalCapture'
import { useTerminalEvents } from '../application/useTerminalEvents'
import { useQuickCommands } from '../application/useQuickCommands'
...
import { connectProfile, connectLocalTerminal, forgetAiTermKnownHost, disconnectTerminal, terminalResize, terminalSessionActive, terminalWrite } from '../api'
import { saveConnectionProfile } from '../../connections/api'
import type { AuthEndpoint, ConnectionProfile } from '../../connections/types'
import type { CommandHistoryEntry, CommandRecordedEvent, TerminalInputEvent, TerminalInputSyncState, TerminalInputWriteFailureEvent, TerminalInputWriteSource, TerminalOutputEvent, TerminalSelectionEvent } from '../model/events'
import { isSensitiveCommand } from '../../../shared/security/commandPrivacy'
import { attachShellIntegration } from '../model/shellIntegration'
...
import { SYSTEM_TERMINAL_CJK_FALLBACK, WINDOWS_TERMINAL_CJK_FALLBACK, withCjkFallback } from '../model/terminalFont'
...
let sessionId = ''
const { attachTerminalEvents, detachTerminalEvents } = useTerminalEvents({
  sessionId: () => sessionId,
  ingestTerminalOutput,
  captureOutput: data => ingestCaptureOutput(data),
...
const { agentCapturePreparing, agentCaptureSupported, ensureAgentCapture, runCommandAndCapture, ingestCaptureOutput, abortActiveCapture } = useTerminalCapture({
  getSessionId: () => sessionId,
  getShellIntegration: () => shellIntegrationAttachment,
