...
assert(
  styles.includes('[role="button"],') &&
    ...
    chromeSelection.includes('selectableTextSelector') &&
    chromeSelection.includes('handleAppSelectStart') &&
    chromeSelection.includes("document.addEventListener('selectstart', handleAppSelectStart, true)") &&
    chromeSelection.includes("document.removeEventListener('selectstart', handleAppSelectStart, true)") &&
    chromeSelection.includes('handleAppDragStart') &&
    chromeSelection.includes("document.addEventListener('dragstart', handleAppDragStart, true)") &&
    chromeSelection.includes('clearChromeSelection(target?: EventTarget | null)') &&
    chromeSelection.includes('selectionEndpointElement') &&
    chromeSelection.includes('clearChromeSelection(event.target)'),
...
assert(
  connectionProfileState.includes('selectedProfile = computed(() =>') &&
    connectionProfileState.includes('return undefined') &&
...
assert(
  terminalPane.includes('commandHistory: CommandHistoryEntry[]') &&
    ...
    terminalInputRouter.includes('writeSyncedTerminalInput(event.data, event.terminalId)') &&
    ...
    terminalEvents.includes('ingestTerminalOutput(event.data)') &&
...
assert(
  terminalPane.includes("type TerminalInputContext = 'shell' | 'sensitive' | 'unknown'") &&
    ...
    terminalHandleTypes.includes('writeSyncedTerminalInput: (data: string, sourceTerminalId: string) => boolean'),
...
const appTerminalSyncBlock = terminalInputRouter.slice(
  terminalInputRouter.indexOf('function syncTerminalInputToTargets'),
  terminalInputRouter.indexOf('function handleTerminalInputWriteFailure')
)
const appSelectTerminalBlock = sourceSection(terminalTabState, 'function selectTerminalTab', 'function addTerminalTab')

assert(
  terminalEventTypes.includes('export interface TerminalInputSyncState') &&
    terminalEventTypes.includes('export interface TerminalInputWriteFailureEvent') &&
    terminalEventTypes.includes("'interactive' | 'direct' | 'synced' | 'command'") &&
    ...
    terminalInputRouter.includes('terminalInputSyncStatesMatch') &&
    terminalInputRouter.includes('pauseTerminalSyncTargets') &&
...
assert(
  shellCommand.includes('export function detectShellScriptLanguage') &&
    ...
    scriptEditorRules.includes('bashTokenPattern') &&
    scriptEditorRules.includes('powershellTokenPattern') &&
    scriptEditorRules.includes('cmdTokenPattern') &&
    scriptEditorRules.includes('highlightShellLine(line, language)') &&
...
assert(
  scriptEditorRules.includes('normalizeScriptEditorContent(content)') &&
    scriptEditorRules.includes("content.replace(/\\r\\n?/g, '\\n')") &&
...
assert(
  scriptReadiness.includes('analyzeScriptReadiness') &&
    ...
    scriptExecutionState.includes("import { analyzeScriptReadiness }") && scriptPanel.includes("import { scriptReadinessStatusForContent }") &&
...
assert(
  sidebar.includes('v-model="selectedProfile.target.host"') &&
    ...
    profileConfigRules.includes("normalized.connectionRole = normalized.connectionRole === 'bastion' ? 'bastion' : 'direct'") &&
    profileConfigRules.includes("normalized.gateway = {") &&
    profileConfigRules.includes("normalized.jumpMode = 'direct'") &&
    profileConfigRules.includes("normalized.fileTransferMode = 'auto'") &&
...
assert(
  sidebar.includes("return 'SSH 端口'") &&
    ...
    profileConfigRules.includes("normalizePort(normalized.target.port, 'SSH port', 22)"),
...
assert(
  sidebar.includes("copy: [profileId: string]") &&
    ...
    connectionProfileState.includes('function copySelectedProfile') &&
    appShell.includes('@copy="copySelectedProfile"') &&
    appShell.includes("id: 'copy'") &&
    connectionProfileState.includes('nextConnectionProfileId') &&
    connectionProfileState.includes('nextConnectionProfileName') &&
...
assert(
  !appShell.includes('structuredClone(') &&
    connectionProfileState.includes('cloneConnectionProfile') &&
...
assert(
  connectionProfileState.includes("jumpMode: 'direct'") &&
    connectionProfileState.includes("connectionRole: 'direct'") &&
    connectionProfileState.includes("menuProfileId: ''"),
...
assert(
  connectionApi.includes('listConnectionProfiles') &&
    connectionApi.includes('saveConnectionProfile') &&
    connectionApi.includes('deleteConnectionProfile') &&
    appShell.includes('await listConnectionProfiles()') &&
    connectionProfileState.includes('await saveConnectionProfile(normalizedProfile)') &&
    connectionProfileState.includes('await deleteConnectionProfile(profileId)') &&
...
assert(
  workspacePanel.includes('activeWorkspaceTab = computed(() => props.activeTab)') &&
    ...
    aiConversationContext.includes('generateAiSessionTitle') &&
    aiPanel.includes('maybeGenerateSessionTitle') &&
    aiConversationContext.includes("emit('updateSessionTitle'") &&
...
assert(
  terminalTabState.includes('selectedTerminalIds') &&
    ...
    terminalInputRouter.includes('for (const delay of COMMAND_EXECUTION_RETRY_DELAYS_MS)') &&
    terminalInputRouter.includes("readiness === 'ready' && pane?.writeTerminalInput(data)") &&
...
assert(
  aiPanel.includes('chatWithAiProvider') &&
    ...
    aiPanelTypes.includes('terminalSelection?: TerminalSelectionEvent') &&
    ...
    aiPanelTypes.includes('messages: AiMessage[]') &&
    ...
    aiConversationRules.includes('MAX_AI_CONVERSATION_MESSAGES') &&
    ...
    scriptApi.includes("invoke<UpdateScript[]>('list_update_scripts'") &&
    scriptApi.includes("invoke<void>('save_update_script'") &&
    scriptApi.includes("invoke<boolean>('delete_update_script'") &&
    aiApi.includes("invoke<WorkspaceSession[]>('list_workspace_sessions'") &&
    aiApi.includes("invoke<void>('save_workspace_session'") &&
    aiApi.includes("invoke<boolean>('delete_workspace_session'") &&
    aiApi.includes("invoke<AiSessionTitleResponse>('generate_ai_session_title'") &&
    tauri.includes("invoke<CommandHistoryEntry[]>('list_command_history'") &&
    tauri.includes("invoke<void>('save_command_history_record'") &&
    aiApi.includes("invoke<AiMessage[]>('list_ai_conversation_messages'") &&
    aiApi.includes("invoke<void>('save_ai_conversation_message'") &&
    aiApi.includes('export function listWorkspaceSessions()') &&
    tauri.includes('export function listCommandHistory(connectionId: string)') &&
    aiApi.includes('export function listAiConversationMessages(workspaceSessionId: string)') &&
    scriptApi.includes('export function listUpdateScripts()') &&
    ...
    agentSession.includes('cancelTask') &&
    ...
    aiApi.includes('chatWithAiProvider'),
...
assert(
  workspacePanel.includes('ScriptPanel') &&
    ...
    scriptRecordingState.includes('scriptRecordingsByTerminal') &&
    ...
    scriptGeneration.includes('terminalSnapshot: recordedOutput()') &&
    ...
    scriptGeneration.includes('chatWithAiProviderStream') &&
    ...
    scriptGeneration.includes('createdAt: new Date().toISOString()') &&
    ...
    scriptPresentation.includes('extractBashScript') &&
    ...
    scriptExecutionState.includes('buildBashScriptTerminalInput(prepared)') &&
    scriptPanel.includes('../../../shared/security/scriptRisk') &&
    scriptPresentation.includes('analyzeScriptRisks') &&
    ...
    scriptApi.includes("invoke<AiScriptTitleResponse>('generate_ai_script_title'") &&
...
assert(
  scriptPreviewStorage.includes("const PREVIEW_SCRIPT_STORAGE_KEY = 'ai-term:update-scripts:v2:global'") &&
    scriptPanel.includes('migratePreviewScripts') &&
    scriptLibrary.includes('await listUpdateScripts()') &&
    scriptPanel.includes('pendingScriptConnectionMismatch') &&
    scriptPanelTypes.includes('executionTargetConnectionIds') &&
    scriptPanel.includes('scriptSourceConnectionId') &&
    scriptPanel.includes('draftSourceConnectionId') &&
    scriptPanelTypes.includes('sourceConnectionId?: string') &&
...
assert(
  workspaceSessionRules.includes('const COMMAND_HISTORY_CACHE_LIMIT = 300') &&
    ...
    aiConversationRules.includes('const MAX_AI_COMMAND_HISTORY = 80') &&
    aiConversationContext.includes('.slice(-MAX_AI_COMMAND_HISTORY)') &&
    aiConversationContext.includes('.filter((command) => !isSensitiveCommand(command))') &&
    ...
    packageManifest.scripts?.['test:scripts']?.includes('tests/shared/command-privacy.test.mjs') &&
...
assert(
  terminalPane.includes('quick-command-bar') &&
    ...
    quickCommands.includes('scriptRiskStatusForContent') &&
    quickCommands.includes("scriptRiskStatusForContent(value).level === 'high'") &&
    ...
    quickCommands.includes('pinQuickCommand(command: string)') &&
    ...
    quickCommands.includes("props.profile?.id || 'local'") &&
...
assert(
  aiConfig.includes('apiKey:') &&
    ...
    aiConfigState.includes('aiConfigs = ref') &&
    appShell.includes('selectedAiConfigId') &&
    appShell.includes('createAiConfig') &&
    aiConfigState.includes('aiRuntimeApiKeys') &&
    aiConfigState.includes('listAiProviderConfigs') &&
    aiConfigState.includes('saveAiProviderConfig') &&
    aiConfigState.includes('deleteAiProviderConfig') &&
...
function aiPanelUsesBackendModelCall() {
  ...
  return (
    ...
    aiConversationRules.includes('error = false') &&
    aiApi.includes("invoke<AiChatResponse>('chat_with_ai_provider_stream'") &&
    aiApi.includes('onAiChatStream') &&
    aiConversationRules.includes('extractPrimaryShellCommand')
  )
}
...
assert(
  sidebar.includes('shouldShowTargetPassword') &&
    ...
    sidebar.includes('SSH 密码') &&
    sidebar.includes('v-model="selectedProfile.target.password"') &&
    profileConfigRules.includes('if (!normalized.target.password?.trim()) normalized.target.password = undefined') &&
    profileConfigRules.includes("normalized.connectionRole = normalized.connectionRole === 'bastion' ? 'bastion' : 'direct'") &&
    profileConfigRules.includes("normalized.jumpMode = 'direct'") &&
    profileConfigRules.includes("normalized.fileTransferMode = 'auto'"),
...
assert(
  aiPanel.includes('explainPendingAiCommandRisk') &&
    ...
    scriptGeneration.includes('onAiChatStream(requestId') &&
    scriptExecutionState.includes('streamedAnswer += event.delta') &&
...
assert(
  transferPresentation.includes('浏览器预览中无法使用本地文件和 SFTP 能力，请在 AI Term 桌面端中操作。') &&
    transferPresentation.includes('isTauriPreviewUnavailable') &&
...
assert(
  ...
  /printf '\\\\n\$\{MARKER_PREFIX\}%s__\\\\n' \$\{nonce\}B/.test(sentinelCapture) &&
    ...
    terminalCapture.includes('captureWithSentinel') &&
    terminalPane.includes('captureOutput: data => ingestCaptureOutput(data)') && terminalEvents.includes('options.captureOutput(event.data)') && terminalCapture.includes('sentinelSink?.(data)') &&
    terminalCapture.includes('isSuffixSafeForSentinel') &&
...
assert(
  ...
  !/远端需 shell 自行上报 OSC 133/.test(appShell) &&
    ...
    /captureWithSentinel\(SENTINEL_PROBE_COMMAND, /.test(terminalCapture),
...
assert(
  ...
  agentStepCard.includes('props.step.deadlineAt') &&
    ...
    /watch\(agentRunActive/.test(agentSession) &&
...
assert(
  ...
  /handle\.peekOutput\(\)/.test(agentLoop) &&
    ...
    /stepLimit: props\.agentStepLimit/.test(agentSession) &&
    /commandTimeoutMs: props\.agentCommandTimeoutMs/.test(agentSession) &&
