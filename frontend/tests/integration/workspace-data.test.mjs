...
test('Agent 载荷恢复合法连接代次，非法或旧值保留原消息的代次', () => {
  for (const terminalConnectionGeneration of [0, 1, 42, Number.MAX_SAFE_INTEGER]) {
    const hydrated = hydrateAiMessagePayload(message('agent', {
      terminalConnectionGeneration: 7,
      payloadJson: JSON.stringify({ mode: 'agent', agentStatus: 'stopped', terminalConnectionGeneration })
    }))
    assert.equal(hydrated.terminalConnectionGeneration, terminalConnectionGeneration)
  }
  for (const terminalConnectionGeneration of [undefined, null, -1, 1.5, '3', Number.MAX_SAFE_INTEGER + 1]) {
    const payloadJson = JSON.stringify({ mode: 'agent', terminalConnectionGeneration })
    assert.equal(hydrateAiMessagePayload(message('agent', { terminalConnectionGeneration: 7, payloadJson })).terminalConnectionGeneration, 7)
    assert.equal(hydrateAiMessagePayload(message('legacy', { payloadJson })).terminalConnectionGeneration, undefined)
  }
})
...
test('会话存储加载 Agent 消息时恢复绑定的终端及连接代次', async context => {
  ...
  storage.listAiConversationMessages = async () => [message('agent', {
    terminalId: 'local-1',
    payloadJson: JSON.stringify({ mode: 'agent', agentStatus: 'stopped', agentSteps: [], terminalConnectionGeneration: 3 })
  })]
  ...
  const restored = messages.aiMessagesBySession.value.one[0]
  assert.equal(restored.terminalId, 'local-1')
  assert.equal(restored.terminalConnectionGeneration, 3)
  assert.equal(restored.agentStatus, 'stopped')
})
